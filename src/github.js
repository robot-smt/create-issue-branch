...
async function getBranchName(ctx, config, title) {
    ...
    return (
        utils.makePrefixGitSafe(getIssueBranchPrefix(ctx, config), {
            replaceChars: replaceChars, //
            replacementChar: replacementChar //
        }) +
        utils.makeGitSafe(result, {
            replaceChars: replaceChars,
            replacementChar: replacementChar
        })
    )
...
async function getCommitTreeSha(ctx, commitSha) {
    const owner = context.getRepoOwnerLogin(ctx)
    const repo = context.getRepoName(ctx)
    const res = await ctx.octokit.git.getCommit({
        owner,
        repo,
...
async function createBranch(ctx, config, branchName, sha, log) {
    ...
    try {
        ...
        const commentMessage = interpolate(Config.getCommentMessage(config), {
            ...ctx.payload,
...
async function copyIssueAssigneeToPr(ctx, pr) {
    ...
    await ctx.octokit.issues.addAssignees({
        owner,
        repo,
        issue_number: pr.number,
        assignees: [assignee]
...
async function queryProjectIdsForIssue(ctx) {
  ...
  `
    const queryResult = await ctx.octokit.graphql(queryProjectIds, {
        owner: context.getRepoOwnerLogin(ctx),
        repo: context.getRepoName(ctx),
        number: context.getIssueNumber(ctx)
    })
